...
def train_net(model, img_dir, max_iter = 700000, check_every_n = 50, loss_check_n = 10, save_model_freq = 500, batch_size = 256):
	...
	validate_img_saver = Img_Saver(test_img_saver_dir)
	...
	if testing == True:
		idx = np.arange(2)
		test_batch_files = [testing_images_list[i] for i in idx]
		print test_batch_files
		[images1, images2] = load_image(dir_name = testing_img_dir, img_names = test_batch_files)
		[latent1, latent2] = get_latent_var(images1, images2)
		[reconst1, reconst2] = model.siamese_decoder(latent1, latent2)
		# [reconst1, reconst2, latent1, latent2] = get_reconst_img(images1, images2)
		for i in range(len(latent1[0])):
			print "{} th: {} : {}".format(i, latent1[0, i], latent2[0, i])
		...
		# 	save_img = np.squeeze(images1[img_idx])
		# 	save_img = Image.fromarray(save_img)
		# 	img_file_name = "{}_ori.png".format(test_batch_files[img_idx].split('.')[0])				
		# 	test_img_saver.save(save_img, img_file_name, sub_dir = sub_dir)

		# 	save_img = np.squeeze(reconst1[img_idx])
		# 	save_img = Image.fromarray(save_img)
		# 	img_file_name = "{}_rec.png".format(test_batch_files[img_idx].split('.')[0])				
		# 	test_img_saver.save(save_img, img_file_name, sub_dir = sub_dir)